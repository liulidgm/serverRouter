...
function filter() {
    require('../lib/log').info('first step')
    return function (req, res, next) {
        ...
        //     //     return next();
        //     // }
        //     // res.render('tmpl/mobile.html');
        //     // return res.send("11111111111111111111111");
        // } else {

            /*每个页面请求都把用户信息请求下来*/
            // client.do.initIndex({}, req, res, function (err, result) {
            //     if (err) return next();
            //     Logger.info('进入登录验证');
            //     Logger.info(req.cookies);
            //     Logger.info(result);
            //     if (result.code == '999' || !req.cookies.phone) { //没登录
            //         Logger.info('未登录或者未绑卡或者未评测');
            //         req.user = {
            //             code: result.code || '999',
            //             userName: '',
            //             mobile: ''
            //         };
